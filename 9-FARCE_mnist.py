...
parser.add_argument('-d', '--dataset', default='mnist', help='dataset name')
...
def main():
    ...
    model_clser = models.__dict__['resnet20_mnist'](16, 10)
    ...
    if os.path.isfile(',/mnist/checkpoint_stage_res20.pth.tar'):
        print("=> loading checkpoint '{}'".format('./mnist/checkpoint_stage_res20.pth.tar'))
        checkpoint = torch.load(',/mnist/checkpoint_stage_res20.pth.tar')
        model_clser.load_state_dict(checkpoint['state_dict_m'])
        print("=> loaded checkpoint '{}' (epoch {})"
              .format('./mnist/checkpoint_stage_res20.pth.tar', checkpoint['epoch']))
    else:
        print("=> no checkpoint found at '{}'".format(',/mnist/checkpoint_stage_res20.pth.tar'))
...
def train(train_loader, model_netG, model_netD, model_clser, optimizerG, optimizerD, epoch, criterion_CEL):
    ...
    for i, (input, target, index) in enumerate(train_loader):
        ...
        if (i + 1) % 5 == 0:
            ...
            predicted_labels, _ = model_clser(x_fake)
    ...
    if epoch > 98:
        ...
        if not os.path.exists("./mnist/demo/" + "/source_image"):
            os.makedirs("./mnist/demo/" + "/source_image")
        if not os.path.exists("./mnist/demo/" + "/target_image"):
            os.makedirs("./mnist/demo/" + "/target_image")
        if not os.path.exists("./mnist/demo/" + "/delta_image"):
            os.makedirs("./mnist/demo/" + "/delta_image")
        ...
        for i, (input, target, index) in enumerate(train_loader):
            ...
            for i_iter in range(10-1):
                ...
                for i_img in range(c_trg.size(0)):
                    ...
                    save_path = "./mnist/demo/" + "/source_image/" + str(class_org) + "_" + str(class_trg) + "_" + str(index[i_img].data.cpu().numpy()).zfill(5) + ".png"
                    vutils.save_image(x_real_tmp[i_img].data, save_path, nrow=1)

                    save_path = "./mnist/demo/" + "/target_image/" + str(class_org) + "_" + str(class_trg) + "_" + str(index[i_img].data.cpu().numpy()).zfill(5) + ".png"
                    vutils.save_image(x_fake[i_img].data, save_path, nrow=1)

                    save_path = "./mnist/demo/" + "/delta_image/" + str(class_org) + "_" + str(class_trg) + "_" + str(index[i_img].data.cpu().numpy()).zfill(5) + ".png"
                    ...
                    final_fake_delta_image_B[fake_delta_image > threshold] = fake_delta_image[
                        fake_delta_image > threshold]
                    final_fake_delta_image_B[fake_delta_image < -threshold] = 0
                    final_fake_delta_image_R = x_real_tmp[i_img].data.cpu().numpy().squeeze()
                    final_fake_delta_image_R[fake_delta_image < -threshold] = -fake_delta_image[
                        fake_delta_image < -threshold]
                    ...
                    final_fake_delta_image_G[fake_delta_image < -threshold] = 0
                    final_fake_delta_image[:, :, 0] = final_fake_delta_image_B
                    final_fake_delta_image[:, :, 2] = final_fake_delta_image_R
