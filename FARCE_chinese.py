...
parser.add_argument('-d', '--dataset', default='chinese1', help='dataset name')
...
def main():
    ...
    # create model
    model_netG = cfNet.Generator()
    model_netD = cfNet.Discriminator()
    model_clser = models.__dict__['resnet20'](16, 10)
    ...
    if os.path.isfile('./chinese1/checkpoint_pretrain_res20.pth.tar'):
        print("=> loading checkpoint '{}'".format('./chinese1/checkpoint_pretrain_res20.pth.tar'))
        checkpoint = torch.load('./chinese1/checkpoint_pretrain_res20.pth.tar')
        model_clser.module.load_state_dict(checkpoint['state_dict_m'])
        print("=> loaded checkpoint '{}' (epoch {})"
              .format('./chinese1/checkpoint_pretrain_res20.pth.tar', checkpoint['epoch']))
    else:
        print("=> no checkpoint found at '{}'".format('./chinese1/checkpoint_pretrain_res20.pth.tar'))
...
def train(train_loader, model_netG, model_netD, model_clser, optimizerG, optimizerD, epoch, criterion_CEL):
    ...
    for i, (input, target, index) in enumerate(train_loader):
        ...
        c_org = label2onehot(label_org, 10)
        c_trg = label2onehot(label_trg, 10)
    ...
    if epoch > 98:
        ...
        if not os.path.exists("./chinese1/demo/" + "/source_image"):
            os.makedirs("./chinese1/demo/" + "/source_image")
        if not os.path.exists("./chinese1/demo/" + "/target_image"):
            os.makedirs("./chinese1/demo/" + "/target_image")
        if not os.path.exists("./chinese1/demo/" + "/delta_image"):
            os.makedirs("./chinese1/demo/" + "/delta_image")
        ...
        for i, (input, target, index) in enumerate(train_loader):
            ...
            for i_iter in range(10-1):
                label_trg_tmp = label_trg_tmp - 1
                label_trg_tmp[label_trg_tmp < 0] = 9
                ...
                for i_img in range(c_trg.size(0)):
                    ...
                    save_path = "./chinese1/demo/" + "/source_image/" + str(class_org) + "_" + str(class_trg) + "_" + str(index[i_img].data.cpu().numpy()).zfill(5) + ".png"
                    vutils.save_image(x_real_tmp[i_img].data, save_path, nrow=1)

                    save_path = "./chinese1/demo/" + "/target_image/" + str(class_org) + "_" + str(class_trg) + "_" + str(index[i_img].data.cpu().numpy()).zfill(5) + ".png"
                    vutils.save_image(x_fake[i_img].data, save_path, nrow=1)

                    save_path = "./chinese1/demo/" + "/delta_image/" + str(class_org) + "_" + str(class_trg) + "_" + str(index[i_img].data.cpu().numpy()).zfill(5) + ".png"
                    ...
                    # final_fake_delta_image[:, :, 0] = x_real_tmp[i_img].data.cpu().numpy().squeeze()
                    # final_fake_delta_image[:, :, 1] = x_real_tmp[i_img].data.cpu().numpy().squeeze()
                    # final_fake_delta_image[:, :, 2] = x_real_tmp[i_img].data.cpu().numpy().squeeze()
                    final_fake_delta_image_B = x_real_tmp[i_img].data.cpu().numpy().squeeze()
                    final_fake_delta_image_B[fake_delta_image > threshold] = fake_delta_image[fake_delta_image > threshold]
                    final_fake_delta_image_B[fake_delta_image < -threshold] = 0
                    final_fake_delta_image_R = x_real_tmp[i_img].data.cpu().numpy().squeeze()
                    final_fake_delta_image_R[fake_delta_image < -threshold] = -fake_delta_image[fake_delta_image < -threshold]
                    final_fake_delta_image_R[fake_delta_image > threshold] = 0
                    final_fake_delta_image_G = x_real_tmp[i_img].data.cpu().numpy().squeeze()
                    final_fake_delta_image_G[fake_delta_image > threshold] = 0
                    final_fake_delta_image_G[fake_delta_image < -threshold] = 0
